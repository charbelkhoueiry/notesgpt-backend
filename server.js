...
// Only the modified/new files since the first working version
const updatesOnly = {
  "server.js": `
    const express = require("express");
    const cors = require("cors");
...
// Path to write updated files
const updatesPath = "/mnt/data/notesgpt-backend-updates";
if (!fs.existsSync(updatesPath)) {
  fs.mkdirSync(updatesPath);
}

for (const filename in updatesOnly) {
  fs.writeFileSync(path.join(updatesPath, filename), updatesOnly[filename].trim());
}

// Create zip of the update
const updatesZipPath = "/mnt/data/notesgpt-backend-updates.zip";
const output = fs.createWriteStream(updatesZipPath);
const archive = archiver("zip", { zlib: { level: 9 } });

output.on("close", () => console.log(`Zip created: ${updatesZipPath}`));
archive.pipe(output);

fs.readdirSync(updatesPath).forEach((file) => {
  archive.file(path.join(updatesPath, file), { name: file });
});
