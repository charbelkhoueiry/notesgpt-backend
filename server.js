...
updates_path = Path("/mnt/data/notesgpt-backend-updates")
updates_path.mkdir(exist_ok=True)

for filename, content in updates_only.items():
    (updates_path / filename).write_text(content.strip())

# Create zip of the update
updates_zip_path = "/mnt/data/notesgpt-backend-updates.zip"
with zipfile.ZipFile(updates_zip_path, "w") as zf:
    for file in updates_path.glob("*"):
